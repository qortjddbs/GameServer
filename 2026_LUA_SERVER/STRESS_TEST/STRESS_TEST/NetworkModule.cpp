...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			case 1: if (g_clients[i].y < WORLD_HEIGHT - 1) g_clients[i].y++; break;
			case 2: if (g_clients[i].x > 0) g_clients[i].x--; break;
			case 3: if (g_clients[i].x < WORLD_WIDTH - 1) g_clients[i].x++; break;
			}
