...
void SESSION::do_random_move()
{
	...
	if (_id == MAX_USER) {
		...
		std::cout << "NPC " << _id << " moved. Delay : " << duration_cast<milliseconds>(delay).count() << "ms\n";
	}
		...
		if (timer_queue.try_pop(ev)) {
			...
			if (ev.wakeup_time <= now) {
				...
				case EVENT_MOVE:
					...
					PostQueuedCompletionStatus(h_iocp, -1, ev.obj_id, &move_over->_over);		// 두번째 인자는 number of bytes transferred인데, 0을 넣으면 커넥션이 끝났다고 판단할 수 있기 때문에 0말고 다른 값
					break;
				}
			}
			else {
				// 이래도 돌아가긴 하지만 성능적으로 바람직하지 않음. why? timer_queue는 여러 쓰레드가 건드리니까 이거 access하는 거 오버헤드가 크다. 
