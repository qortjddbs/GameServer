...
void process_packet(int c_id, char* packet)
{
	...
	case C2S_LOGIN: {
		...
		WCHAR query[256];
		swprintf_s(query, 256, L"EXEC sp_login_check N'%hs'", p->username);

		g_db_lock.lock();
		SQLRETURN retcode = SQLExecDirect(hStmt, query, SQL_NTS);

		bool login_success = false;
		SQLINTEGER db_x = 0, db_y = 0;

		if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
			SQLLEN cbX = 0, cbY = 0; 
			SQLBindCol(hStmt, 1, SQL_C_LONG, &db_x, 0, &cbX);
			SQLBindCol(hStmt, 2, SQL_C_LONG, &db_y, 0, &cbY);

			SQLRETURN fetchRet = SQLFetch(hStmt);
			if (fetchRet == SQL_SUCCESS || fetchRet == SQL_SUCCESS_WITH_INFO) {
				login_success = true; // DB에 유저가 존재
			}
			else if (fetchRet != SQL_NO_DATA) {
				// 못 찾은 게 아니라, 데이터를 꺼내다가 에러가 난 경우
				std::wcout << L"\n[에러] SQLFetch 중 문제 발생!" << std::endl;
				HandleDiagnosticRecord(hStmt, SQL_HANDLE_STMT, fetchRet);
			}
		}
		else {
			std::wcout << L"\n[에러] 쿼리 실행 실패: " << query << std::endl;
		...
		if (!login_success) {
			S2C_LoginResult fail_p;
			fail_p.size = sizeof(fail_p);
			fail_p.type = S2C_LOGIN_RESULT;
			fail_p.success = false;
			strcpy_s(fail_p.message, "Login Failed: ID not found in DB.");
			clients[c_id]->do_send(&fail_p);
...

		{
			...
			clients[c_id]->x = static_cast<short>(db_x);
			clients[c_id]->y = static_cast<short>(db_y);
...
void disconnect(int c_id)
{
	...
	if (clients[c_id]->_state == ST_INGAME && !is_npc(c_id)) {
		SQLHSTMT hStmt;
		SQLAllocHandle(SQL_HANDLE_STMT, g_hDbc, &hStmt);

		WCHAR query[256];
		// sp_save_position 프로시저 호출 (ID, X, Y)
		swprintf_s(query, L"EXEC sp_save_position N'%hs', %d, %d",
			clients[c_id]->_name, clients[c_id]->x, clients[c_id]->y);

		g_db_lock.lock();
		SQLRETURN retcode = SQLExecDirect(hStmt, query, SQL_NTS);

		if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO) {
			std::wcout << L"\n[에러] 위치 저장 실패: " << query << std::endl;
			HandleDiagnosticRecord(hStmt, SQL_HANDLE_STMT, retcode);
		}
...
int main()
{
	...
	SQLRETURN db_ret = SQLConnect(g_hDbc, (SQLWCHAR*)L"2026_game", SQL_NTS, NULL, 0, NULL, 0);

	if (db_ret == SQL_SUCCESS || db_ret == SQL_SUCCESS_WITH_INFO) {
		std::wcout << L"========== [서버 DB 접속 성공!] ==========" << std::endl;
	}
	else {
		std::wcout << L"========== [서버 DB 접속 실패...] ==========" << std::endl;
		HandleDiagnosticRecord(g_hDbc, SQL_HANDLE_DBC, db_ret);
		system("pause");
		return -1;
	}
