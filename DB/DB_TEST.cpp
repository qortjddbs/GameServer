    ...
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
        ...
        if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
            ...
            if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
                ...
                retcode = SQLConnect(hdbc, (SQLWCHAR*)L"2026_game", SQL_NTS, (SQLWCHAR*)NULL, 0, NULL, 0);
                ...
                if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
                    ...
                    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
                        ...
                        retcode = SQLBindCol(hstmt, 1, SQL_C_SLONG, &user_id, 10, &cb_id);
                        retcode = SQLBindCol(hstmt, 2, SQL_C_WCHAR, user_name, NAME_LEN, &cb_name);
                        retcode = SQLBindCol(hstmt, 3, SQL_C_SLONG, &user_level, 10, &cb_level);
