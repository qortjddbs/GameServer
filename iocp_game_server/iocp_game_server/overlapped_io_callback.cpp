...
public:
	...
	void do_recv()
	{
		...
		m_recv_over.m_client_id = m_id;
		WSARecv(m_client, &m_recv_over.m_wsa, 1, 0, &recv_flag, &m_recv_over.m_over, recv_callback);
	}
	void do_send(int num_bytes, char* mess)
	{
		...
		WSASend(m_client, &o->m_wsa, 1, 0, 0, &o->m_over, send_callback);
	}
...
void CALLBACK recv_callback (DWORD err, DWORD num_bytes, LPWSAOVERLAPPED recv_over, DWORD recv_flag)
{
	EXP_OVER* o = reinterpret_cast<EXP_OVER*>(recv_over);
	short player_index = o->m_client_id;
	...
	unsigned char* p = reinterpret_cast<unsigned char*>(o->m_buff);
...
void CALLBACK send_callback(DWORD err, DWORD num_bytes, LPWSAOVERLAPPED send_over, DWORD send_flag)
{
	EXP_OVER* o = reinterpret_cast<EXP_OVER*>(send_over);
	delete o;
}
...
int main()
{
	...
	//WSAEVENT listen_event = WSACreateEvent();			// 센서 달기
	//WSAEventSelect(server, listen_event, FD_ACCEPT);	// FD_ACCEPT (접속 시도)가 발생하면 센서 울림

	for (;;) {
		// DWORD ret = WSAWaitForMultipleEvents(1, &listen_event, FALSE, WSA_INFINITE, TRUE);
		// listen_event가 울릴 때까지 대기. 혹은 콜백 함수 실행

		//if (ret == WAIT_IO_COMPLETION) {			// 콜백 함수를 실행하고 돌아왔을 때
		//	continue;
		//}

		//if (ret == WAIT_OBJECT_0) {				// listen_event가 울렸을 때
		//	WSANETWORKEVENTS net_events;
		//	WSAEnumNetworkEvents(server, listen_event, &net_events);
		...
		SOCKET client_socket = WSAAccept(server, reinterpret_cast<sockaddr*>(&client_addr), &addr_len, NULL, NULL);
		if (client_socket != INVALID_SOCKET) {
			...
			error_display(L"Accept failed", WSAGetLastError());
		}
	}
	closesocket(server);
	WSACleanup();
