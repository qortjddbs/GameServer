...
#include <mutex>
#include <shared_mutex>		// 읽기는 여러명이, 쓰기는 한명만 가능
...
constexpr int MAX_SECTOR_X = WORLD_WIDTH / SECTOR_SIZE;
constexpr int MAX_SECTOR_Y = WORLD_HEIGHT / SECTOR_SIZE;
...
short get_sec_x(short x) { return max(0, min(MAX_SECTOR_X - 1, x / SECTOR_SIZE)); }
short get_sec_y(short y) { return max(0, min(MAX_SECTOR_Y - 1, y / SECTOR_SIZE)); }

std::unordered_set<int> get_near_players(int my_id, short x, short y)
{
	std::unordered_set<int> near_players;
	short sx = get_sec_x(x);
	short sy = get_sec_y(y);
	...
	for (int dy = -1; dy <= 1; ++dy) {
		for (int dx = -1; dx <= 1; ++dx) {
			...
			if (nsx < 0 || nsx >= MAX_SECTOR_X || nsy < 0 || nsy >= MAX_SECTOR_Y) continue;
			std::shared_lock<std::shared_mutex> read_lock(g_sectors[nsx][nsy].lock);

			for (int other_id : g_sectors[nsx][nsy].players) {
				if (other_id != my_id) 
					near_players.insert(other_id);
			}
		}
	}
	return near_players;
...
bool SESSION::process_packet(unsigned char* p)
{
	...
	case C2S_LOGIN: {
		...
		short sx = get_sec_x(m_x);
		short sy = get_sec_y(m_y);
		{
			std::unique_lock<std::shared_mutex> write_lock(g_sectors[sx][sy].lock);
			g_sectors[sx][sy].players.insert(m_id);
		}

		send_avatar_info();
		m_state = CS_PLAYING;

		auto near_pl = get_near_players(m_id, m_x, m_y);
		for (int other_id : near_pl) {
			std::shared_ptr<SESSION> pl = clients[other_id].load();
	...
	case C2S_MOVE: {
		...
		short old_sx = get_sec_x(m_x);
		short old_sy = get_sec_y(m_y);
		...
		short new_sx = get_sec_x(m_x);
		short new_sy = get_sec_y(m_y);

		if (old_sx != new_sx || old_sy != new_sy) {
			{
				std::unique_lock<std::shared_mutex> write_lock(g_sectors[old_sx][old_sy].lock);
				g_sectors[old_sx][old_sy].players.erase(m_id);
			}
			{
				std::unique_lock<std::shared_mutex> write_lock(g_sectors[new_sx][new_sy].lock);
				g_sectors[new_sx][new_sy].players.insert(m_id);
			}
		}
...
void disconnect(int key)
{
	...
	if (nullptr != cl) {
		...
		short sx = get_sec_x(cl->m_x);
		short sy = get_sec_y(cl->m_y);
		{
			std::unique_lock<std::shared_mutex> write_lock(g_sectors[sx][sy].lock);
			g_sectors[sx][sy].players.erase(cl->m_id);
		}
