...
void draw_minimap() {
	// 미니맵 크기 설정
	constexpr float WORLD_MAX = 400.0f;
	constexpr float MINIMAP_SIZE = 200.0f;
	constexpr float SCALE = MINIMAP_SIZE / WORLD_MAX;	// 400좌표를 200픽셀로 줄임 (절반)

	// 우측 상단에 그리기
	float start_x = WINDOW_WIDTH - MINIMAP_SIZE - 20.0f;
	float start_y = 20.0f;

	// 미니맵 설정
	sf::RectangleShape bg(sf::Vector2f(MINIMAP_SIZE, MINIMAP_SIZE));		// 직사각형 틀
	bg.setPosition(start_x, start_y);										// 여기에
	bg.setFillColor(sf::Color(0, 0, 0, 150));								// 반투명으로
	g_window->draw(bg);

	// 내 시야에 들어온 다른 플레이어 (빨간색)
	sf::CircleShape other_dot(2.0f);
	other_dot.setFillColor(sf::Color::Red);
	for (auto& pl : players) {
		if (false == pl.second.m_showing) continue;
		float px = start_x + (pl.second.m_x * SCALE);
		float py = start_y + (pl.second.m_y * SCALE);
		other_dot.setPosition(px - 2.0f, py - 2.0f); // 점의 중심을 좌표에 맞춤
		g_window->draw(other_dot);
	}

	// 내 캐릭터 (초록색)
	sf::CircleShape my_dot(1.0f);		// 다른 플레이어들이 잘 보이게끔. 나는 작게
	my_dot.setFillColor(sf::Color::Green);
	float mx = start_x + (avatar.m_x * SCALE);
	float my = start_y + (avatar.m_y * SCALE);
	my_dot.setPosition(mx - 3.0f, my - 3.0f);
