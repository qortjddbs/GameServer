...
#include <thread>
#include <vector>
#include <unordered_set>		// id만 저장하면 되니까 map까지는 필요없음
#include "protocol.h"
...
constexpr int VIEW_RANGE = 5;		// 16 x 16 칸이라 5칸정도로 설정
...
public:
	...
	short m_x, m_y;
	int m_move_time;
	std::unordered_set<int, int> m_visible_players;
	...
	SESSION(SOCKET s, int id) : m_client(s), m_id(id) {
		...
		m_prev_recv = 0;
		m_move_time = 0;		// 사실 굳이 초기화 할 필요는 없음.
	}
	~SESSION()
	{
	...
	void send_remove_player(int player_id)
	{
		...
		if (m_visible_players.count(player_id) == 0) {
			...
			return;		// 이미 보이는 플레이어라면 패킷을 보내지 않는다.
		}
	...
	bool process_packet(unsigned char* p);		// 혹시라도 이상한 패킷 (해킹으로 인해)이 오면 false를 반환해 접속 차단.
...
bool SESSION::process_packet(unsigned char* p)
{
	...
	case C2S_LOGIN: {
		...
		for (auto& c : clients) {
			...
			if (false == is_visible(pl->m_x, pl->m_y)) continue;		// pl->m_id 해도 되지만 오버헤드가 크기때문에 좌표만.
...
void SESSION::send_move_packet(int mover)
{
	...
	std::shared_ptr<SESSION> pl = clients[mover];			// 지역 변수이기 때문에 atomic일 필요가 없다. 쓰레드가 공유 안함.
	if (nullptr == pl) return;		// shared_ptr이기 때문에 nullptr이 아니면 안심하고 써도 됨. 실행 중 destruct가 될 수 없음.
	packet.x = pl->m_x;
	packet.y = pl->m_y;
...
void disconnect(int key)
{
	...
	if (nullptr != cl) {
		...
		auto visible_copy = cl->m_visible_players;		// disconnect가 되면 다른 쓰레드에서 m_visible_players를 수정할 수 있기 때문에 복사본을 만들어서 사용한다.
	...
	for (;;) {
		...
		case IO_RECV:
		{
			...
			while (data_size > 0) {
				...
				if (false == cl->process_packet(p)) {		// process_packet에서 invalid packet이 나오면 disconnect.
					disconnect(key);
					break;
				}
				p += packet_size;
