...
void SESSION::send_add_player(int player_id)
{
	...
	m_visible_mutex.lock();	//락 걸어야함. 
	if(m_visible_players.count(player_id) > 0) {	//이미 보이는 플레이어면 안보내도 됨
		m_visible_mutex.unlock();
		return;
	}
...
void SESSION::do_move(DIRECTION dir)
{
	...
	send_move_packet(m_id);	//자기 자신한테도 이동 패킷을 보내야한다. (자기 자신이 움직였으니까)

	for (int id : new_v) {
		if (old_v.count(id) == 0) {
			//새로 보이는 플레이어한테는 추가 패킷을 보낸다.
			send_add_player(id);
...
bool SESSION::process_packet(unsigned char* p)
{
	...
	case C2S_LOGIN: {
		...
		for (auto& c : clients) {	//모든 클라에 알릴 필요는 없다
			std::shared_ptr<SESSION> pl = c.second.load();
			if (nullptr == pl) continue;
			if (pl->m_id == m_id) continue;
			if (false == is_visible(pl->m_x, pl->m_y)) continue;	// 보이는 범위에 있는 플레이어한테만 알려주면 됨.
			if (pl->m_state != CS_PLAYING) continue;
...
void disconnect(int key)
{
	...
	if (nullptr != cl) {
		...
		for (auto& other : cl->m_visible_players) {	//모든 클라에 보낼 필요 없다.
